...
import TomographyOracleCore.Revision.MWPLSOracle
import TomographyOracleCore.Revision.ConstantRates

/-! MW-PLS expectation and physical-risk transport. The integration and discrete
rate reduction follow the existing generic algorithm interface; the optimization
premise is the new quadratic first-order certificate, not the OMD objective. -/

namespace TomographyOracleCore.MWPLS
open MeasureTheory ProbabilityTheory MatrixReduction PhysicalPOVM PhysicalRisk Revision.PhysicalMinimax
open scoped BigOperators Matrix.Norms.L2Operator ENNReal
noncomputable section
set_option maxHeartbeats 1200000
variable {n K T : ℕ}
theorem periodicAlgorithmExpectedError_oracle_mw (h : ChoKimBlockCondition n K) (T : ℕ)
    (solve : Matrix (Fin (2 ^ n)) (Fin (2 ^ n)) ℂ → DensityOperator (Fin (2 ^ n)))
    (gamma : ℝ) (hgamma : 0 ≤ gamma)
    (hfit : ∀ Q (rho : DensityOperator (Fin (2 ^ n))),
      pairing (finiteUnitaryFullCalibratedLinearChannel
        (choKimPeriodicTwoLayerCliffordUnitaryFin h.block_dvd) (solve Q).matrix-Q)
        ((solve Q).matrix-rho.matrix) ≤ gamma^2)
  ...
  have hpoint : ∀ᵐ sample ∂periodicSampleLaw h T rho,
    ...
    have hsampleFit : HasFirstOrderCertificate U (empiricalForwardMatrix sample)
        (algorithmSampleEstimator hD U T solve sample) (gamma^2) := by
      rw [algorithmSampleEstimator_eq hD U T solve sample hsample]
      exact hfit _
    have ho := periodic_certificate_oracle h (empiricalForwardMatrix sample)
      (algorithmSampleEstimator hD U T solve sample) rho gamma hgamma hsampleFit
    have hor := ho r hr1 hrd
    dsimp only [algorithmTraceError, periodicForwardError, forwardObjective]
    nlinarith
...
/-- The literal expected risk has the advertised three-regime rate when
the first-order gap is at most gamma squared and gamma is at most sqrt(D/T). -/
theorem periodicAlgorithmExpectedError_rate_mw (h : ChoKimBlockCondition n K)
    (hT : 0 < T)
    (solve : Matrix (Fin (2 ^ n)) (Fin (2 ^ n)) ℂ → DensityOperator (Fin (2 ^ n)))
    (gamma : ℝ) (hgamma : 0 ≤ gamma)
  ...
  have horacle := periodicAlgorithmExpectedError_oracle_mw h T solve gamma hgamma hfit
    rho (A * scale) hmean
...
def algorithmUpperConstant_mw (c A : ℝ) : ℝ := max (2 * c) (10 * max 1 ((2 / sharpPeriodicCurvature) * A + (1 / sharpPeriodicCurvature)))

theorem algorithmUpperConstant_mw_pos (c A : ℝ) (hc : 1 ≤ c) :
    0 < algorithmUpperConstant_mw c A := by
  exact lt_of_lt_of_le (by linarith : 0 < 2 * c) (le_max_left _ _)

/-- Pointwise all-sample physical risk of the supplied actual solver. -/
theorem periodicAlgorithm_physicalRisk_rate_mw (h : ChoKimBlockCondition n K)
    (hT : 0 < T)
    (solve : Matrix (Fin (2 ^ n)) (Fin (2 ^ n)) ℂ → DensityOperator (Fin (2 ^ n)))
    (gamma : ℝ) (hgamma : 0 ≤ gamma)
    (hgammaScale : gamma ≤ Real.sqrt (((2 ^ n : ℕ) : ℝ) / (T : ℝ)))
    ...
    ENNReal.ofReal (algorithmUpperConstant_mw c A *
  ...
  · have herr := periodicAlgorithmExpectedError_rate_mw h hT solve gamma hgamma hgammaScale
...
theorem periodicAlgorithm_simultaneousPhysicalUpper_mw (h : ChoKimBlockCondition n K)
    (hT : 0 < T)
    (solve : Matrix (Fin (2 ^ n)) (Fin (2 ^ n)) ℂ → DensityOperator (Fin (2 ^ n)))
    (gamma : ℝ) (hgamma : 0 ≤ gamma)
    (hgammaScale : gamma ≤ Real.sqrt (((2 ^ n : ℕ) : ℝ) / (T : ℝ)))
    ...
    SimultaneousPhysicalUpper (2 ^ n) T
      (constantMatrixPOVMPhysicalDesign (choKimPeriodicFiniteUnitaryProjectivePOVM h.block_dvd) T)
      (fun _ => algorithmUpperConstant_mw c A) := by
  ...
  apply iSup_le
  intro rho
  exact periodicAlgorithm_physicalRisk_rate_mw h hT solve gamma hgamma hgammaScale hfit
