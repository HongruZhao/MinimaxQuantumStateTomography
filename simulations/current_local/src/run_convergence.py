...
from run_identity import (cache_identity, check_cached_result, file_hash, source_hashes,
...
def run(config, task):
    ...
    if status_path.exists():
        ...
        if prior.get('source_sha256') != source_hashes() or prior.get('config_sha256') != config_hash:
            raise StaleResultError('Source/configuration changed; use a new convergence run name.')
        if not data_path.is_file() or not (out / 'sampling.json').is_file():
            raise StaleResultError('Recorded convergence input is incomplete.')
        identity = cache_identity(dict(config=config, task=task),
                                  {'data.npz': file_hash(data_path),
                                   'sampling.json': file_hash(out / 'sampling.json')})
        if check_cached_result(status_path, identity) is not None:
            return
    elif data_path.exists():
        raise StaleResultError('Orphaned data cannot be silently reused without its source/configuration record.')
    ...
    row['cache_identity'] = cache_identity(dict(config=config, task=task),
                                         {'data.npz': file_hash(data_path),
                                          'sampling.json': file_hash(out / 'sampling.json')})
    row['status'] = 'solving'
    ...
    for T, b in zip(case['sizes'], means):
        ...
        for method in METHODS:
            ...
            check = independent_validation(c, b, T, x, method, stats, rho=rho,
                                           tol_factor=config.get('solver_options', {}).get('tol_factor', 1.))
    ...
    row['artifact_sha256'] = artifact_hashes(out, ('*.npz', 'sampling.json'))
