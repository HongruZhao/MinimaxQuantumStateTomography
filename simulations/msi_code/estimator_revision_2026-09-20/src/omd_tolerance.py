...
def solve(c,y,T,alpha=1.7,beta=1.,regfrac=.01,maxiter=12000,tolfrac=.01,return_witness=False):
    ...
    for j in range(1,maxiter+1):
        ...
        if j%20==0:
            ...
            if dg['gap']<=tolfrac*gamma and dg['regularized_gap']<=regfrac*gamma:break
    dg=certificate(c,y,z,-beta*u,eta)
    stats=dict(dg,seconds=time.perf_counter()-start,iterations=j,converged=bool(dg['gap']<=tolfrac*gamma and dg['regularized_gap']<=regfrac*gamma),history=hist)
